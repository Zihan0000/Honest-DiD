...
example_rm<-run_simulation_rm(1)
...
#ratio
merged_RM2<- merge(example_rm, optEL_df_results, by = "pulse_multiplier", all.x = TRUE)
merged_RM2 <- merged_RM2 %>%
  mutate(efficiency_rm = Opt_EL / mean_EL) 
saveRDS(merged_RM2, file = "merged_RM2_avg.rds")
...
plot = ggplot(merged_RM2 %>% filter(Mbar == 3), 
...
plot
ggsave(plot, 
       file = here("Figures/Figure_rm2_avg.png"), 
