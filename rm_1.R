...
plot = ggplot(merged_RM %>% filter(Mbar == 1), 
  ...
  ggtitle(latex2exp::TeX("$\\Delta^{RM}(\\bar{M})$, $\\theta = {\\tau_1}$")) +
  labs(x = latex2exp::TeX("$\\delta_{-1}$"), y = "Excess length ratio", 
       colour = "", shape = "", linetype = "")
plot
ggsave(plot, 
       file = here("Figures/Figure_rm_11.png"), 
